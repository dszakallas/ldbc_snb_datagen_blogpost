...
LDBC SNB provides [Datagen](https://github.com/ldbc/ldbc_snb_datagen) (Data Generator), which produces synthetic datasets, mimicing a social network's activity during a period of time. Datagen is defined by the charasteristics of realism, scalability, determinism and usability. To address scalability in particular, Datagen is implemented on the MapReduce platform to enable scaling out across a distributed cluster. However, since its inception in the early 2010s there have been a tremendous amount of development in the big data landscape. This has lead to a reevaluation of this implementation, investigating if Apache Spark would now be a more appropriate underlying platform.
...
Entities are generated by procedural Java code and are represented as both POJOs in memory and as sequence files on disk. Most entities follow a shallow representation, i.e foreign keys in the ER model are mapped to integer ids, which makes serialization straightforward.[<sup>1</sup>](#fn1) One exception from this is the `Knows` edge which contains only the target node, and is used as a navigation property on the source `Person`. The target `Person` is replaced with only the foreign key augmented with some additional information in order to keep the structure free of cycles. Needless to say, this _edge as property_ representation makes the data more cumbersome to handle in SQL than it would be with a flat join table.
...
**Small entry cost** Spark uses HDFS under the hood, and the two frameworks are very close conceptually. Migration to Spark can, therfore, be completed with relatively small effort. For example, MapReduce and Spark jobs can be run on AWS EMR using basically the same hw/sw configuration, which leverages straightforward performance comparisons.
...
The first milestone is a successful run of LDBC Datagen on Spark, whilst making the minimum necessary amount of code alterations. This entails the migration of the Hadoop wrappers around the generators and serializers.
